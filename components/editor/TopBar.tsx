...
import { ChevronDown, FolderOpen, Redo2, RotateCcw, Undo2 } from "lucide-react";
import { useEditorStore } from "@/store/useEditorStore";

interface Props { filename: string; onOpen: () => void }

export function TopBar({ filename, onOpen }: Props) {
  const { undo, redo, reset, history, future } = useEditorStore();
  return (
