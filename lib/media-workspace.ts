...
type LocalAccount = { user: { id: string; username: string }; workspace: { id: string; name: string } };

function mediaAppId() {
  const value = process.env.MEDIA_LIBRARY_APP_ID?.trim();
  if (!value) throw new Error("MEDIA_LIBRARY_APP_ID não está configurado.");
  return value;
}
...
export async function ensureMediaWorkspace(account: LocalAccount) {
  ...
  try {
    const mediaWorkspace = await mediaPrisma.mediaWorkspace.create({
      data: { displayName: account.workspace.name, accounts: { create: { appId, externalWorkspaceId: account.workspace.id, externalUserId: account.user.id, username: account.user.username } } },
